...
<div style="border:1px solid #eee;height:480px;padding-left:15px;display:none;width:100%;margin-left:25px;width:80%;margin-bottom:15px;font-size:12px;" id="choose_download_forms" class="row "><br />
            ...
            @if($client->fb_client_id !='' and ($provider->freshbooks_clients_invoice == '1' || $provider->freshbooks_clients_enabled == '1') and $provider->freshbooks_api_url != '' and $provider->freshbooks_api_token != '')
                @if(is_array($client->fb_invoice))
                    @if(is_array($client->fb_invoice['invoice']) && array_key_exists('invoice_id', $client->fb_invoice['invoice']) )
                        ...
                        <div class="row form-group">
                            <div class="col-sm-12">
                                <input type="checkbox" class="download-forms-cb" name="download_invoice" id="download_forms_{{$key}}" value="Client Invoice" />

                                <a href="{{action('ClientController@getInvoicePDF', [$client->id,$provider->id])}}" target="_blank">Client Invoice</a>
...
<div style="border:1px solid #eee;height:480px;padding-left:15px;display:none;margin-left:25px;margin-bottom:15px;font-size:12px;" id="choose_sign_forms" class="row form-group">
    ...
    */
    ...
    @if(is_array($client->fb_invoice['invoice']) && array_key_exists('invoice_id', $client->fb_invoice['invoice']) )
        ...
        {{ Form::hidden('do_custom_invoice', 'y') }}
                    ...
                    @include('providers.invoice', array(
                        'client' => $client,
                        'provider' => $provider
                    ))
